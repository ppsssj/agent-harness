...
13. **Lock direction and hand off.** After actual human approval, set status to `APPROVED` and record concise approval evidence or requested changes. Only an `APPROVED` or `SKIPPED_BY_EXPLICIT_USER_INSTRUCTION` lock with complete element and Major Visual Decision Maps may hand off its synthesis to [frontend-design](../frontend-design/SKILL.md); follow it with visual verification. [DESIGN.md](../../templates/DESIGN.md) may be finalized after approval, but its recorded decisions must come from the approved pre-implementation synthesis.
